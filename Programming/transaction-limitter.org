...
Breaking this rule is never a good idea, because there will be consequences(punishment).
...
Note that I made the base class let the priority tag optional, rather than enforcing the priority tag to be /always/ present but rather 
...
A [[file:circular-buffer.org][circular buffer]] is an essential data structure Transaction Limiter needs to use.
