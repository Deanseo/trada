...
I don't think we can find the prebuilt package of that in Windows. To use Emacs 29 in terminal on Windows, unfortunately, we have to build Emacs on MSys2 from scratch.
#+begin_tip
...
That's it. Just don't forget to use the GNU Emacs to build from.
