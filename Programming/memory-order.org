...
Good news is that if you aren't interested in writing a "portable" lock-free algorithm, which I'm not, you can probably ignore this
...
Yet, in my opinion I didn't witness a case where a consume is used in order to yield a better performance result myself though.

** ✅ Relaxed
:PROPERTIES:
:CUSTOM_ID: -relaxed
:END:
Since x86 doesn't support a relaxed load and store in the hardware level, our concern narrows down to the compiler optimization where we make sure of a valid
/release-acquire,/ /synchronize-with/ relation.

** ✅ Release / Acquire
:PROPERTIES:
:CUSTOM_ID: -release--acquire
:END:
In most cases this pair is good enough for what we want to accomplish.

Each operation is a specific partial ordering for a store and load pair.
In the C++ standard, they must be paired from each thread to be visible to the counterpart thread without the modification order affected (synchronized-with).
...
In which case it's probably advisible to enhance the correct ordering.
...
As such, you can tell that "*r0 = r1 = 0*" is impossible.
...
#+begin_tip
In x86, a normal relaxed atomic operation isn't supported, being escalated to either release or acquire at least.
#+end_tip
