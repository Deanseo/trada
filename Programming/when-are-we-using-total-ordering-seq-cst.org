...
* Still, isn't a rel/acq pair good enough to prevent reordering?
...
The C++'s memory model essentially provides the same definition. When paired, the 'acquire-er' can have the same ordering
so we often use a =is_ready= practice:
...
In most cases this is good enough. Considering x86 is strongly ordered and for sequencially consistent operation the compiler yields stronger
instructions than relaxed, such as =lock= prefix or =mfence=, I once wondered if there's a case we /must/ use =std::memory_order_seq_cst=.
