...
* Table of Contents
- [[#when-are-we-using-total-ordering][When are we using total ordering?]]
  - [[#stdmemory_order_consume][std::memory_order_consume]]
  - [[#stdmemory_order_relaxed][std::memory_order_relaxed]]
  - [[#stdmemory_order_release--stdmemory_order_acquire][std::memory_order_release / std::memory_order_acquire]]
  - [[#stdmemory_order_seq_cst][std::memory_order_seq_cst]]
...
:PROPERTIES:
:CUSTOM_ID: when-are-we-using-total-ordering
:END:
...
** consume
:PROPERTIES:
:CUSTOM_ID: stdmemory_order_consume
:END:
...
https://stackoverflow.com/questions/58986135/what-does-strongly-happens-before-mean: about consume in comments
https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2018/p0668r5.html: scfix
https://www.reddit.com/r/cpp/comments/15v8ke2/is_sequentiallyconsistent_ordering_documentation: reddit

** relaxed
:PROPERTIES:
:CUSTOM_ID: stdmemory_order_relaxed
:END:
...
** release / acquire
:PROPERTIES:
:CUSTOM_ID: stdmemory_order_release--stdmemory_order_acquire
:END:
...
** seq_cst
:PROPERTIES:
:CUSTOM_ID: stdmemory_order_seq_cst
:END:
...
In x86, a normal relaxed atomic operation is automatically escalated to either release or acquire at least.
So given that A is 'always' /synchronized-with/ B thorugh a rel/acq pair, it's impossible to demonstrate a valid usage of seq_cst
even from a perspective of C which isn't in the rel/acq chain.
