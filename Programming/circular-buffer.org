...
This implementation can be also used in my previous post about [[file:transaction-limitter.org][Transaction Limitter]].

Therefore, it's useful to know the basics of Circular Buffer.
There are several ways to implement this, depending on the requirement such as:

- Is it thread-safe?
- Are there multiple producers/consumers accessing the buffer?  
