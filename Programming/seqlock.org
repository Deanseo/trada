...
In other words, using a relaxed operation on =seq_= (or a naive `int` type) with a compiler barrier is probably sufficient if we don't care about keeping it portable.
