...
:END:
*seqlock* is a simple and fask locking mechanism that doesn't block the writer.
That trait is especially useful when a thread has to constantly feeds tick data into other algo carriers.

Not only that, it's often used to solve many /real-world/ problems and in fact part of the kernel in Linux as well.
