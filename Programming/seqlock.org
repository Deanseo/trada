...
#+BEGIN_SRC C++ :includes <atomic> :flags -std=c++20 :results silent raw :exports both :main no :noweb yes
  ...
  // rax == s0
  mov     rax, QWORD PTR [rsp+128]  # _58,
  // if (s0 & 1)
  test    al, 1   # _58,
  jne     .L27      #,
  // value_.load(std::memory_order_relaxed)
  mov     edx, DWORD PTR [rsp+136]  # _61,
  // ref.store(..., std::memory_order_relaxed)
  mov     DWORD PTR [rsp+120], edx  #,, _61
  // if inlined, rax and rdx registers can be used to check if the load is valid directly such that:
  // cmp rax rdx
  mov     rdx, QWORD PTR [rsp+128]  # _62,
#+END_SRC
